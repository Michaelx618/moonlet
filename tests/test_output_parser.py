...
def test_strip_diff_one_liner():
    ...
    text = "```diff --- a/foo.c +++ b/foo.c @@ -1,5 +1,6 @@ #include <stdio.h>```"
...
def test_extract_oneliner_code_block():
    ...
    output = (
        "Here is the updated `foo.c`: "
        "```c #include <stdio.h> #include <stdlib.h> int main(void) { return 0; } ```"
    )
    blocks = extract_markdown_lang_only_blocks(output, candidate_paths=["src/foo.c"])
    assert len(blocks) == 1
    path, body = blocks[0]
    assert path == "src/foo.c"
...
def test_infer_path_from_reply_text():
    """Path should be inferred from 'the `foo.c` file' in surrounding text."""
    # Block is in the middle; before/after contain the mention
    output = "To complete the `foo.c` file we need to add: ```c int x; ```"
    block_start = output.find("```c")
    block_end = output.find("```", block_start + 5) + 3
    path = _infer_path_from_reply_text(output, block_start, block_end, ["src/foo.c", "Makefile"])
    assert path == "src/foo.c"
...
def test_parse_flexible_output_oneliner():
    ...
    kind, diff_data, blocks = parse_flexible_output(output, candidate_paths=["foo.c"])
    assert kind == "blocks"
    assert blocks is not None
    assert len(blocks) >= 1
    path, body = blocks[0]
    assert "foo.c" in path or path == "foo.c"
