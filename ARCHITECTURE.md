...
Client (Electron/local-app)
    │
    │  POST /stream { mode, text, focus_file?, extra_read_files?, plan_context? }
    ▼
┌─────────────────────────────────────────────────────────────────────────┐
│  server.APIServer (server.py)                                           │
│  • Validates path, resolves focus_file                                   │
│  • Optional: find_relevant_files() when no focus_file (agent/ask/plan)   │
│  • If plan_context and mode agent: run_text = "Implement plan" + plan   │
│  • Branches on mode                                                     │
└─────────────────────────────────────────────────────────────────────────┘
    │
    ├── mode == "ask"   ──►  ask_plan.run_ask()      (read-only loop, ASK_TOOLS_HINT)
    ├── mode == "plan"  ──►  ask_plan.run_plan()     (read-only loop, PLAN_TOOLS_HINT)
    └── mode in (agent, repair)  ──►  agent_loop.run_agent()  (full tools, unchanged)
```
...
| **Agent** | `agent_loop.py` | Builds one prompt (workspace paths, reference files, focus file, tools hint, user text). Calls model; parses tool calls from reply; runs tools via `tool_executor`; appends results and repeats until no tool calls. Unchanged for ask/plan. |
| **Ask / Plan** | `ask_plan.py` | Separate routes: `run_ask` and `run_plan` with their own prompts (ASK_TOOLS_HINT, PLAN_TOOLS_HINT) and read-only tool loops. Write tool calls are rejected. |
...
| **Model** | `model.py` | MLX backend only. `stream_reply_chunks()` for completion-style streaming; completion path parses tool calls from text (USE_CHAT_TOOLS not supported with MLX). |
